...
static int command_settings(int tty_fd, CANUSB_SPEED speed, CANUSB_MODE mode, CANUSB_FRAME frame)
{
  ...
  cmd_frame[cmd_frame_len++] = 0; /* Mask ID not handled. */
  cmd_frame[cmd_frame_len++] = 0; /* Mask ID not handled. */
  cmd_frame[cmd_frame_len++] = 0;//mode normal
  cmd_frame[cmd_frame_len++] = 0x04;
  cmd_frame[cmd_frame_len++] = 0x02;
  cmd_frame[cmd_frame_len++] = 0x06;
  cmd_frame[cmd_frame_len++] = 0;
  cmd_frame[cmd_frame_len++] = 0;
  cmd_frame[cmd_frame_len++] = generate_checksum(&cmd_frame[2], 17);
...
static int send_data_frame_ignition_on(int tty_fd)
{
  #define FRAME_SIZE 15
  int data_frame_len = 0;
  unsigned char data_frame[FRAME_SIZE] = {0x00};

  /*Byte 0: Packet start*/
  data_frame[data_frame_len++] = 0xaa;

  /* Byte 1: CAN Bus Data Frame Information extended and 8 bytes od DLC*/
  data_frame[data_frame_len++] = 0xE8;

  /*Byte 2 to 5: CAN Frame ID 10FF0021 (ignition frame for DTHE)*/
  data_frame[data_frame_len++] = 0x21;
  data_frame[data_frame_len++] = 0x00;
  data_frame[data_frame_len++] = 0xFF;
  data_frame[data_frame_len++] = 0x10;

  /*BYTE 6 to 13: 8 bytes of payload*/
  data_frame[data_frame_len++] = 0xF2; /*F2 == on, F0 == off*/
  data_frame[data_frame_len++] = 0xFF;
  data_frame[data_frame_len++] = 0xFF;
  data_frame[data_frame_len++] = 0xFF;
  data_frame[data_frame_len++] = 0xFF;
  data_frame[data_frame_len++] = 0xB9;
  data_frame[data_frame_len++] = 0xFF;
  data_frame[data_frame_len++] = 0x19;
...
static int send_data_frame_ignition_off(int tty_fd)
{
  #define FRAME_SIZE 15
  int data_frame_len = 0;
  unsigned char data_frame[FRAME_SIZE] = {0x00};
  ...
  /*BYTE 6 to 13: 8 bytes of payload*/
  data_frame[data_frame_len++] = 0xF0; /*F2 == on, F0 == off*/
  data_frame[data_frame_len++] = 0xFF;
  data_frame[data_frame_len++] = 0xFF;
  data_frame[data_frame_len++] = 0xFF;
  data_frame[data_frame_len++] = 0xFF;
...
static int inject_data_frame(int tty_fd, const char *hex_id, const char *hex_data)
{
  ...
  while (program_running && ! error) {
    ...
    {
      error = send_data_frame_ignition_off(tty_fd);
    }
    else
    {
      error = send_data_frame_ignition_on(tty_fd);
